...
const Music1 = ({video, idx, idSearch,pointerEvents, mood, albums, playlist, idArtist, imArtist}) => {
  ...
  return (
    <div className="music1-music" style={video?.videoId ? {opacity: '0.6', marginLeft: '0px', marginRight: '0px'} : null || {pointerEvents}}>
      <Link id={video?.id} to={ mood === 'youtube' ? (video?.videoId ? `/video/${video?.videoId}` : null || video?.id ? `/video/${idSearch}` : null) : null || 
        ...
        className="button music-line music1-line"
           onClick={() => { 
                  ...
                  mood === 'spotify' ? (playlist === '0' && video?.id ?  Cookies.set('spotifyType', '123:'+video?.type+":"+video?.id) :
                          (Cookies.set('idSongPlayList',[video?.id,"0", video?.album?.images?.[0]?.url,"0", video?.name,"0", video?.artists?.[0]?.name,"0",
                          millisToMinutesAndSeconds(video?.duration_ms),"0", video?.album?.artists?.[0]?.id,"0",video?.popularity]),Cookies.set('idChannel', video?.album?.artists?.[0]?.id), Cookies.set('playlistActivate', '1'))) : null ||
                  mood === 'appleMusic' ? (playlist === '0' && video?.id ? video?.id &&  Cookies.set('spotifyType', '123:'+video?.value?.attributes?.type+":"+video?.id) : (Cookies.set('idSongPlayList',
                    [video?.playParams?.id,"0", imArtist,"0", video?.name,"0", video?.artistName,"0", video?.releaseDate,"0", idArtist]),
                    Cookies.set('idChannel', idArtist), Cookies.set('playlistActivate', '1'))) : null}}>
        <span id="number" className="music1-text">#{++idx }
        </span>
        <img
          alt='sadasda'
          src={video?.thumbnail?.[0]?.url || video?.thumbnail?.url  || video?.thumb || albums || video?.album?.images?.[0]?.url || video?.images?.coverarthq 
            || video?.value?.attributes?.images?.coverArtHq || image}
          className="music1-image"
        />
        <span id="song" className="music1-text01">
          <span> {video?.channelTitle || video?.author || video?.channel?.name || video?.artists?.[0]?.name || video?.[0]?.artists?.[0]?.name || video?.subtitle || 
            video?.artistName || video?.value?.attributes?.primaryArtist }
          </span>
          <br></br>
        </span>
        <span className="music1-text04">
          <span >{ video?.title ? video?.title.slice(0,50) : null || video?.name || video?.[0]?.name || video?.value?.attributes?.title}</span>
          <br></br>
        </span>
        { mood !== 'appleMusic' ? <svg viewBox="0 0 1024 1024" className="music1-icon">
        ...
        { mood !== 'appleMusic' ? <svg viewBox="0 0 1024 1024" className="music1-icon2">

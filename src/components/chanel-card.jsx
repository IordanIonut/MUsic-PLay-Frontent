...
const ChanelCard = ({channelDetail, idx, mood}) => {
  ...
  return(
      ...
      <div className="button music-line"><span>{idx}</span>
        ...
        <span className="music-text04" style={{marginLeft: '50px'}}>{channelDetail?.channelTitle || channelDetail?.[0]?.id?.snippet?.title || channelDetail?.data?.profile?.name}</span>
        <span className="music-text04" style={{marginLeft: '50px'}}>{channelDetail?.subscriberCount || channelDetail?.[0]?.id?.statistics?.subscriberCount}</span>
        </div>
        </Link>
      </div>
    )

...
console.log(arrayDB)
  return(
      ...
      {mood === 'youtube' && idSp && type === 'video' && Array.isArray(arrayDB) && arrayDB.map((item, idx) => (
            ...
            {  Array.isArray(arrayDB) && item?.content_id.mood ==='youtube'  &&  <Music color={item?.fill}  video={item?.content_id?.description} idx={idx}></Music> }
            </section>
          ))}
      {mood === 'youtube'  && idSp && type === 'playlist' && Array.isArray(arrayDB) && arrayDB.map((item, idx) => (
            ...
            {Array.isArray(arrayDB) && item?.content_id.mood ==='youtube' &&   <FeatureCard playlist={item?.content_id?.description} idx={idx}></FeatureCard> }
            </section>
          ))}
      {mood === 'youtube'  && idSp && type === 'channel' && Array.isArray(arrayDB) && arrayDB.map((item, idx) => (
            <section key={idx} style={{width: '99%' , transitionDelay: '1s' }}> 
            {Array.isArray(arrayDB) && item?.content_id.mood ==='youtube' &&  <ChanelCard channelDetail={item?.content_id?.description} idx={idx}></ChanelCard> }
            </section>
          ))}
      {mood === 'spotify' && idSp && type === 'video' && Array.isArray(arrayDB) && arrayDB.map((item, idx) => (
            <section  key={idx} style={{width: '99%', transitionDelay: '1s' }}> 
            {  Array.isArray(arrayDB) && item?.content_id.mood ==='spotify'  &&  <Music color={item?.fill}   video={item?.content_id?.description} idx={idx}></Music> }
            </section>
          ))}
      {mood === 'spotify'  && idSp && type === 'playlist' && Array.isArray(arrayDB) && arrayDB.map((item, idx) => (
            <section key={idx} style={{marginLeft: '', transitionDelay: '1s'}}> 
            {Array.isArray(arrayDB) && item?.content_id.mood ==='spotify' &&  <FeatureCard playlist={item?.content_id?.description} idx={idx}></FeatureCard> }
            </section>
          ))}
      {mood === 'spotify'  && idSp && type === 'channel' && Array.isArray(arrayDB) && arrayDB.map((item, idx) => (
            <section key={idx} style={{width: '99%' , transitionDelay: '1s' }}> 
            {Array.isArray(arrayDB) && item?.content_id.mood ==='spotify' &&  <ChanelCard channelDetail={item?.content_id?.description} idx={idx}></ChanelCard> }
            </section>
          ))}
    {mood === 'appleMusic' && idSp && type === 'video' && Array.isArray(arrayDB) && arrayDB.map((item, idx) => (
            <section  key={idx} style={{width: '99%', transitionDelay: '1s' }}> 
            {  Array.isArray(arrayDB) && item?.content_id.mood ==='appleMusic'   &&  <Music   color={item?.fill}  video={item} idx={idx}></Music> }
            </section>
          ))}
      {mood === 'appleMusic'  && idSp && type === 'playlist' && Array.isArray(arrayDB) && arrayDB.map((item, idx) => (
            <section key={idx} style={{marginLeft: '', transitionDelay: '1s'}}> 
            {Array.isArray(arrayDB) && item?.content_id.mood ==='appleMusic'   &&  <FeatureCard playlist={item?.content_id?.description} idx={idx}></FeatureCard> }
            </section>
          ))}
      {mood === 'appleMusic'  && idSp && type === 'channel' && Array.isArray(arrayDB) && arrayDB.map((item, idx) => (
            <section key={idx} style={{width: '99%' , transitionDelay: '1s' }}> 
            {Array.isArray(arrayDB) && item?.content_id.mood ==='appleMusic'   &&  <ChanelCard channelDetail={item?.content_id?.description} idx={idx}></ChanelCard> }
            </section>
          ))}
      </div>
    </section>

...
const ChanelBar = ({channelDetail,videos, live, playlist, mood}) => {
  ...
  useEffect(()=>{
    if(mood === 'appleMusic'){
      if(videos?.albums !== undefined  || videos?.songs !== undefined || videos?.artists !== undefined){
        const itemList = Object.keys(videos?.albums).map(key => ({
          id: key,
          value: videos?.albums[key]
        }));
        const itemList1 = Object.keys(videos?.songs).map(key => ({
          id: key,
          value: videos?.songs[key]
        }));
        const itemList2 = Object.keys(videos?.artists).map(key => ({
          id: key,
          value: videos?.artists[key]
        }));
        setVideo(itemList1);
        setPlay(itemList);
        setArtist(itemList2);
  ...
  },[videos?.albums, videos?.artists, videos?.songs]);

  useEffect(()=>{
    if(mood === 'appleMusic'){
      if(artist !== undefined){
        const a = artist?.[0]?.value?.attributes?.artwork?.url.split('{w}x{h}');
    ...
    <div className="home-artist1">
      <img
        ...
        src={channelDetail?.brandingSettings?.image?.bannerExternalUrl || 
          channelDetail?.data?.artist?.visuals?.avatarImage?.sources?.[0]?.url || 
          channelDetail?.avatar || image}
        loading="lazy"
      ...
      <div className="home-container3" style={{display: 'flex'}}>
        <div style={{display: 'flex',alignItems: 'center'}}>
          ...
          />
          <span className="home-text20 text">{channelDetail?.snippet?.title || channelDetail?.data?.artist?.profile?.name || artist?.[0]?.value?.attributes?.name}</span>
        </div>
        <div className="home-view2 posibili" >
        {mood !== 'appleMusic' ? <svg xmlns="http://www.w3.org/2000/svg" className="home-icon062" viewBox="0 0 16 16">
      ...
      {mood==='spotify' && type === 'playlist'  && Array.isArray(videos?.data?.artist?.discography?.popularReleases?.items)  && videos?.data?.artist?.discography?.popularReleases?.items.map((item, idx) => (
       ...
       {mood==='appleMusic' && type === 'video' && Array.isArray(video) && video.map((item, idx) => (
            <section key={idx} style={{transitionDelay: '1s'} && (type === 'video' ? {width: '100%'}: null)}>
              <Music video={item?.value?.attributes} treding={'1'} treding1={'1'} idx={idx} page='0' mood={mood}></Music>
            </section>
      ))}
      {mood==='appleMusic' && type === 'playlist'  && Array.isArray(play)  && play.map((item, idx) => (
            <section key={idx} style={{transitionDelay: '1s'} && (type === 'playlist' ? {marginLeft: ''} : null)}>
              <FeatureCard playlist={item?.value?.attributes} mood={mood} idx={idx} ></FeatureCard>
            </section>
      ))}
      </div>
  </section>
